...
const ACCOUNT_NAME_RULES: Record<Region, { main: string[]; preorder: string[]; extras: string[] }> = {
  ...
  BR: {
    // Fátima Scofield — visão única (soma as 3 contas, todas BRL).
    // main = "Fátima Scofield" (a conta sem sufixo B2C/B2B).
    main: ['scofield'],
    preorder: [],
    // extras somadas ao total: "Fátima Scofield - B2C" e "Fátima Scofield - B2B".
    extras: ['b2c', 'b2b'],
  },
};
...
export async function fetchAdCreativeImage(adId: string): Promise<string | undefined> {
  ...
  const fields = 'creative{image_url,thumbnail_url,object_story_spec{link_data{picture},video_data{image_url}},asset_feed_spec{images{url}}}';
